...
class NesEnv(gym.Env, utils.EzPickle):
    ...
    def _step(self, action):
        if 0 == self.is_initialized:
            return self._get_state(), 0, self._get_is_finished(), True, {}
                ...
                if loop_counter >= 20000:
                    ...
                    if self.subprocess is not None:
                        ...
                        try:
                            ...
                            pass
                        self.subprocess = None
                    return self._get_state(), 0, True, False, {'ignore': True}

        # Getting results
        reward = self._get_reward()
        state = self._get_state()
        is_finished = self._get_is_finished()
        is_alive = self._get_is_alive()
        info = self._get_info()

        if not is_alive:
            self._close()

        return state, reward, is_finished, is_alive, info
...
class MetaNesEnv(NesEnv):
    ...
    def _step(self, action):
        ...
        obs, step_reward, is_finished, is_alive, info = NesEnv._step(self, action)
        ...
        return obs, reward, is_finished, is_alive, info
